...
const validateCred = arr => {
    ...
    for (let i = 0; i < data.length; i++){
        ...
        if (newData[i] > 9) {
            newData[i] = newData[i] - 9;
        }
    }
    const totalValue = newData.reduce((accmulator, currentValue) => accmulator + currentValue, startValue);
    if (totalValue % 10 === 0 ) {
        return true;
    } else {
        return false;
...
const idInvalidCardCompanies = inValidCards => {
    ...
    for (let i = 0; i < inValidCards.length; i++){
        firstNumber.push(inValidCards[i].slice(0,1));
        switch (firstNumber){
            case 3:
